...
if ! [[ -f "Server-Files-$PROJECT_VERSION.zip" ]]; then
  rm -fr rm -fr config defaultconfigs resources libraries kubejs scripts overrides mods packmenu Simple.zip forge* manifest.json
  ...
  forge_version="${minecraft_version}-$(jq -r '.minecraft.modLoaders[] | select(.id | contains("forge")) | .id' /data/manifest.json | awk -F"-" '{print $2}')"
